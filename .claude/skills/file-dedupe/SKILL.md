...
3. After apply, run `file-verify` on the journal — the apply is not
   complete until it passes — then re-run `storage-report` so they see
   the reclaimed space.

Gates (mechanical): `scan`/`plan` refuse a stale or never-classified
catalog; `apply` refuses a plan CSV that wasn't generated by `plan` on
this catalog or was modified since (user edits need `--user-edited-plan`,
