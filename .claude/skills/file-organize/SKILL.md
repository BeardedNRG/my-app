...
```bash
python3 scripts/apply_moves.py --db ~/file-org/catalog.db \
    --plan ~/file-org/move-plan.csv \
    --journal ~/file-org/move-journal.jsonl [--dry-run]
```

Gates (mechanical, no overrides): the plan must have been generated by
`plan_moves.py` on this catalog and be byte-identical since (user edits
require `--user-edited-plan`, audit-logged), and a real apply refuses to
run until a `--dry-run` of that exact plan file has been logged.
...
An apply is NOT complete until `file-verify` passes on its journal. Then
re-run `drive-inventory` on the affected roots and `storage-report`, and
confirm overall state with `file-org`'s `pipeline_status.py` — its
`ALL CLEAR` verdict is the only valid completion claim.
