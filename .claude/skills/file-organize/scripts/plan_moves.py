...
def dest_for(src, root, dest_root, keep_below):
...
def main():
    ...
    def add(kind, category, src, dst_dir, size, root):
        dst = dest_for(src, root, dst_dir, keep_below)
    ...
    # Loose files.
    for path, cat, size, root in con.execute(
            "SELECT path, category, size, root FROM files"
            " WHERE unit_id IS NULL"):
        d = dests.get(cat)
        if not d:
            skipped[f"category '{cat}' unmapped/null"] += 1
            continue
        add("file", cat, path, d, size, root)
